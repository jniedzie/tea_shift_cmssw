#!/usr/bin/env python3
"""Print robust SHIFT momentum-scale summaries by topology and fit strategy."""
...
def main():
    ...
    for event in events:
        for index in range(int(event.nShiftMuon)):
            ...
            if gen_index < 0 or gen_index >= int(event.nGenPart):
                continue
            muon_counts[(topology, "unconstrained")] += 1
            ...
            for quantity, reco, gen in (
                ...
                if value is not None and math.isfinite(value):
                    muon_residuals[(topology, "unconstrained", quantity)].append(value)
            ...
            if p_residual is not None:
                ...
                diagnostic_residuals[(topology, "momentum", momentum_bin)].append(p_residual)
                diagnostic_residuals[(topology, "hit_fit", used_precision)].append(p_residual)
                diagnostic_residuals[(topology, "stations", str(stations))].append(p_residual)

            if not bool(event.ShiftMuon_constrainedValid[index]):
                continue
            muon_counts[(topology, "unconstrained_paired")] += 1
            muon_counts[(topology, "constrained")] += 1
            for quantity, value in unconstrained_values.items():
                muon_residuals[(topology, "unconstrained_paired", quantity)].append(value)
            for quantity, reco, gen in (
                ...
                value = relative(reco, gen)
                if value is not None and math.isfinite(value):
                    muon_residuals[(topology, "constrained", quantity)].append(value)
        ...
        for index in range(int(event.nShiftDimuonVertex)):
            ...
            if first_gen < 0 or second_gen < 0 or first_gen == second_gen:
                continue
            topologies = tuple(sorted((int(event.ShiftMuon_topology[first]), int(event.ShiftMuon_topology[second]))))
            ...
            dimuon_counts[(topologies, "unconstrained")] += 1
            unconstrained_values = {}
            for quantity, reco, gen in (
                ...
                value = relative(reco, gen)
                if value is not None and math.isfinite(value):
                    dimuon_residuals[(topologies, "unconstrained", quantity)].append(value)
                    unconstrained_values[quantity] = value

            if not (bool(event.ShiftMuon_constrainedValid[first]) and
                    bool(event.ShiftMuon_constrainedValid[second])):
                continue
            dimuon_counts[(topologies, "unconstrained_paired")] += 1
            for quantity, value in unconstrained_values.items():
                dimuon_residuals[(topologies, "unconstrained_paired", quantity)].append(value)
            constrained_pair = (four_vector(event, "ShiftMuon", first, True) +
                                four_vector(event, "ShiftMuon", second, True))
            dimuon_counts[(topologies, "constrained")] += 1
            for quantity, reco, gen in (
                ...
                value = relative(reco, gen)
                if value is not None and math.isfinite(value):
                    dimuon_residuals[(topologies, "constrained", quantity)].append(value)
    ...
    for topology in TOPOLOGY_NAMES:
        for strategy in ("unconstrained", "unconstrained_paired", "constrained"):
            count = muon_counts[(topology, strategy)]
            print(f"{TOPOLOGY_NAMES[topology]:27s} {strategy:13s} matched={count}")
            for quantity in ("pt", "pz", "p", "eta"):
                n, median, half68, outlier = summary(muon_residuals[(topology, strategy, quantity)])
    ...
    for (topology, diagnostic, category), values in sorted(diagnostic_residuals.items()):
        if len(values) < 20:
            continue
        n, median, half68, outlier = summary(values)
        print(f"{TOPOLOGY_NAMES[topology]:27s} {diagnostic:9s} {category:10s} "
              f"{n:5d} {median:+.5f} {half68:.5f} {outlier:.3f}")

    print("\nDimuon residuals from the same two muons: n median half68 outlier(|r|>0.5)")
    for topologies in sorted({key[0] for key in dimuon_counts}):
        label = f"{TOPOLOGY_NAMES[topologies[0]]}+{TOPOLOGY_NAMES[topologies[1]]}"
        for strategy in ("unconstrained", "unconstrained_paired", "constrained"):
            count = dimuon_counts[(topologies, strategy)]
            if count == 0:
                continue
            print(f"{label:39s} {strategy:13s} matched={count}")
            for quantity in ("pt", "pz", "mass"):
                n, median, half68, outlier = summary(dimuon_residuals[(topologies, strategy, quantity)])
                print(f"  {quantity:4s} {n:5d} {median:+.5f} {half68:.5f} {outlier:.3f}")
